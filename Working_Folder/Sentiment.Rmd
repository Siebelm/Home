...
for (i in 1:7) {  
  ...
  corpus <- rbind(corpus, para3)
}
corpus <- corpus[2:nrow(corpus),] # delete first empty row
corpus$Book <- factor(corpus$Book, levels=titles)
...
```{r}
corpus$Wordcount <- sapply(strsplit(corpus$Text %>% as.character(), " "), length)

pasteNQ0("Average Amount of Words per Paragraph Triplet")
summary(corpus$Wordcount)

# Graph distribution of words all
ggplot(corpus, aes(Wordcount, fill=I("maroon"))) + 
  geom_histogram() + 
  stat_bin(bins = 100) +
  Grph_theme() +
...
ggplot(corpus, aes(Wordcount, fill=Book)) + 
  geom_area(stat = "bin") + 
...
```{r}
# Create Dictionaries
heroes <- c("harry","ron","hermione","hagrid","dumbledore","weasley","sirius","lupin","moody","slughorn","dobby","cedric","luna","tonks","mcgonagall","ginny")
pasteNQ("Number of Heroes:", length(heroes))

villians <- c("snape","dudley","malfoy","umbridge","voldemort","quirrel","bane","pettigrew","dementor","karkaroff","greyback","bellatrix","lucius","vernon","petunia","riddle")
...
CleanText <- basicclean(corpus$Text)
corpus$sent_score <- sapply(CleanText, 
                            function(x) getSentiment(x, dictionary = "afinn", 
                                                     scale = TRUE, score.type = average)) 
                            # function(x) getVaderRuleBasedSentiment(x)) 
corpus$sent_cross <- NA
corpus$sent_cross <- ifelse(corpus$sent_score<0, "Negative", corpus$sent_cross)
corpus$sent_cross <- ifelse(corpus$sent_score>0, "Positive", corpus$sent_cross)
corpus$sent_cross <- ifelse(corpus$sent_score==0, "Neutral", corpus$sent_cross)
pasteNQ("Sentiment Levels")
table(corpus$sent_cross)
...
for(string in heroes) {
  ...
  heroes_sent <- rbind(heroes_sent, 
                       c(string, 
                         corpus %>%
                           .[grepl(findstr,CleanText),"sent_score"] %>%
                             average()*100
                       )
  )
...
corpus$CleanText <- CleanText %>%
                    removestopwords() %>%
                    wordstem() %>%
                    removenonalpha()
corpus$id <- row.names(corpus)

# tokenize
it <- itoken(corpus$CleanText, 
				ids = corpus$id, 
				progressbar = FALSE) 
...
utl <- textTinyR::sparse_term_matrix$new(vector_data = unlist(vocab), file_data = NULL,
										                     document_term_matrix = TRUE)
tm <- utl$Term_Matrix(verbose=F)
gl_term_w <- utl$global_term_weights()
dtm <- dtm$doc2vec_methods(method = "idf", global_term_weights = gl_term_w, threads = 3)
...
save(model_df, train, test, train_dv, test_dv, file = "harrypotter_WEidf3.RData")
```


## Neural Network
```{r}
load("harrypotter_WEidf3.RData")
set.seed(0)
...
save(nnet_mod, nnet_pred, nnet_cm, file = "nnet_WEidf3.RData")
```
