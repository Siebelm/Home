...
for (i in 1:8) {  
  ...
  scripts <- rbind(scripts,
                   tibble(Text = rawtext, Film = titles[i], Wordcount = wordcount))
}

# Place film in chronolgical order
scripts$Film <- factor(scripts$Film, levels=titles)
```
...
stm_scripts <- subset(scripts, select = -c(Text, Film, Wordcount))
...
# Top Layer KNN
## final predictions
predict(model_top_knn, pred_scripts, type="raw") 
predict(model_top_knn, pred_scripts, type="prob") 
predict(model_mlogit, pred_scripts)
```
