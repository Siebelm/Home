...
bowtfidf_test <- fit_transform(bow_test, tfidf)
...
# cross validation
cv_tune <- function(data, iteration = 25, num_class = 7, 
                    nrounds = 1, nfold = 5, 
...
pred_train <- pca$x
pred_test <- predict(pca, newdata = pred_test)

# Top layer model
top_train <- xgb.DMatrix(as.matrix(pred_train),
                         label=num_train,
                         missing=NaN)
top_test  <- xgb.DMatrix(as.matrix(pred_test),
                         label=num_test,
                         missing=NaN)
...
```{r}
# cross validation
cv_best <- cv_tune(data = top_train, iteration = 100)
pasteNQ("Best NRound:")
cv_best[[1]] %>% as.numeric()
pasteNQ("Best Params:")
cv_best[[2]]
...
results_test[ , "Top_Label"] <- factor(top_layer_test[ , "Top_Label"],
                                  labels = unique(results_test$Target))
top_acc <- confusionMatrix(top_layer_test$Top_Label, 
                           results_test$Target)
top_acc
...
save(train, test,
     ...
     we_model, we_train,
     sent_acc, bow_acc, bowtfidf_acc, we_acc, top_acc, 
